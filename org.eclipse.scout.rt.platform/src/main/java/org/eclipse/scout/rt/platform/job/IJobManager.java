 ...
 * {@link ISchedulingSemaphore}. That way, jobs which belong to the same semaphore run concurrently until they reach the
 * concurrency level defined for that semaphore. Subsequent tasks then wait in the queue until a permit becomes
 * available.
 *
 * @since 5.1
 */
