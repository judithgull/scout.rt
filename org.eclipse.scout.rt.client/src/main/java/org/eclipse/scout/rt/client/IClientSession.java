...
public interface IClientSession extends ISession, IPropertyObserver {
  ...
  /**
   * Returns the 1-permit {@link ISchedulingSemaphore} to run model jobs of this session in sequence, meaning that only
   * one model job is active at any given time for this session.
   */
  ISchedulingSemaphore getModelJobSchedulingSemaphore();
}
