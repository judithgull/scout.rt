...
public class ModelJobsTest {
  ...
  public void testIsModelJob() {
    ...
    assertFalse(ModelJobs.isModelJob(Jobs.schedule(mock(IRunnable.class), Jobs.newInput()
        ...
        .withSchedulingSemaphore(session2.getModelJobSchedulingSemaphore()))));

    // not a model job (no session on ClientRunContext)
    assertFalse(ModelJobs.isModelJob(Jobs.schedule(mock(IRunnable.class), Jobs.newInput()
        .withRunContext(ClientRunContexts.empty()
            .withSession(null, false))
        .withSchedulingSemaphore(session1.getModelJobSchedulingSemaphore()))));
  ...
  public void testIsModelThread() {
    ...
    when(clientSession1.getModelJobSchedulingSemaphore()).thenReturn(Jobs.newSchedulingSemaphore(1));

    final IClientSession clientSession2 = mock(IClientSession.class);
    when(clientSession2.getModelJobSchedulingSemaphore()).thenReturn(Jobs.newSchedulingSemaphore(1));
  ...
  public void testNewInput() {
    ...
    assertSame(m_clientSession.getModelJobSchedulingSemaphore(), ModelJobs.newInput(runContext).getSchedulingSemaphore());
  }
