...
public class JobListenerBlockedFutureTest {
  ...
  public void testEventsForBlockingJob() {
    ...
    IClientSession clientSession = mock(IClientSession.class);
    when(clientSession.getModelJobSchedulingSemaphore()).thenReturn(Jobs.newSchedulingSemaphore(1));
      ...
      public void run() throws Exception {
        innerFuture.set(Jobs.getJobManager().schedule(new IRunnable() {
          ...
          public void run() throws Exception {
            ...
            JobTestUtil.waitForMutexCompetitors(modelJobInput.getSchedulingSemaphore(), 2); // 2=outer-job + inner-job
          }
        }, modelJobInput.copy()
            .withName("inner")
