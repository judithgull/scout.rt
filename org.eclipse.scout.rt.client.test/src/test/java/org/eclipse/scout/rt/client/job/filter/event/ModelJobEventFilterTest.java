...
public class ModelJobEventFilterTest {
  ...
  public void test() {
    ...
    when(session1.getModelJobSchedulingSemaphore()).thenReturn(Jobs.newSchedulingSemaphore(1));

    IClientSession session2 = mock(IClientSession.class);
    when(session2.getModelJobSchedulingSemaphore()).thenReturn(Jobs.newSchedulingSemaphore(1));
    ...
    event = new JobEvent(mock(IJobManager.class), JobEventType.JOB_STATE_CHANGED,
        new JobEventData().withFuture(Jobs.schedule(mock(IRunnable.class), Jobs.newInput().withRunContext(ClientRunContexts.empty().withSession(session1, false)).withSchedulingSemaphore(session2.getModelJobSchedulingSemaphore()))));
    assertFalse(filter.accept(event));

    // not a model job (no session on ClientRunContext)
    event = new JobEvent(mock(IJobManager.class), JobEventType.JOB_STATE_CHANGED,
        new JobEventData().withFuture(Jobs.schedule(mock(IRunnable.class), Jobs.newInput().withRunContext(ClientRunContexts.empty().withSession(null, false)).withSchedulingSemaphore(session1.getModelJobSchedulingSemaphore()))));
    assertFalse(filter.accept(event));

    // this is a model job (same session on ClientRunContext and mutex)
    event = new JobEvent(mock(IJobManager.class), JobEventType.JOB_STATE_CHANGED,
        new JobEventData().withFuture(Jobs.schedule(mock(IRunnable.class), Jobs.newInput().withRunContext(ClientRunContexts.empty().withSession(session1, false)).withSchedulingSemaphore(session1.getModelJobSchedulingSemaphore()))));
