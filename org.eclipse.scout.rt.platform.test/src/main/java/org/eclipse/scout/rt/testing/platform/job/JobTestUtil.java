...
public class JobTestUtil {
   ...
   * elapsed.
   */
  public static void waitForMutexCompetitors(final ISchedulingSemaphore mutex, final int expectedCompetitorCount) {
    ...
    while (mutex.getCompetitorCount() != expectedCompetitorCount) {
      if (System.currentTimeMillis() > deadline) {
        fail(String.format("Timeout elapsed while waiting for a semaphore-permit count. [expectedPermitCount=%s, actualPermitCount=%s]", expectedCompetitorCount, mutex.getCompetitorCount()));
      }
      Thread.yield();
    }
  }
